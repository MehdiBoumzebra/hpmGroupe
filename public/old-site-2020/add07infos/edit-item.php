...
//$q = "SELECT *,modeles.modele as modelelibelle FROM `vehicules`,modeles WHERE vehicules.id = {$_GET["id"]} and vehicules.modele=modeles.id";
$q = "SELECT *  FROM `vehicules` WHERE vehicules.id = {$_GET["id"]} ";
...
<select name="marque"  onChange="maj('modele','marque');">
<option value="">Choisissez ...
					...
					?>
					<option value="<?= $type["id"] ?>" <?= $row['marque']==$type["id"]?'selected':''?>><?= $type["nom"] ?></option>
					<? } ?>
...
<option value=""> Choisissez une marque d'abord
<option value=""> 
</select>&nbsp;<font color="#FF0000">*</font>
<script>
maj('modele','marque',<?= $row['modele']?>);
//setStuff('<?= $row['marque']?>');
