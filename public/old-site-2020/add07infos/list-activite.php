...
function update_list(data){
	...
	sendData('GET','sort-list-activite.php',data,handleHttpResponse2)

}
</script>
<?
/*$listingCounter=1;
$q = "SELECT * FROM activite  ORDER BY id ASC  ";
$r = mysql_query($q);
while ($row = mysql_fetch_array($r)) {
$query = "UPDATE activite SET pos3 = " . $listingCounter . ",pos4 = " . $listingCounter . ",pos5 = " . $listingCounter . ",pos6 = " . $listingCounter . ",pos7 = " . $listingCounter . ",pos8 = " . $listingCounter . ",pos9 = " . $listingCounter . ",pos10 = " . $listingCounter . " WHERE id = " . $row['id']; 
mysql_query($query);
$listingCounter++;	
}*/
...
$query = "UPDATE activite SET pos".$_GET['sous_menu']." = ".$_GET['posprec']." WHERE id = " . $_GET['id']; 
mysql_query($query) or die(mysql_error()); 

$query = "UPDATE activite SET pos".$_GET['sous_menu']." = ".$_GET['posact']." WHERE id = " . $_GET['precedent']; 
mysql_query($query) or die(mysql_error()); 
...
<input type="hidden" name="cmd" value="list-activite" id="cmd">
...
$q = "SELECT * FROM activite  where 1 ";

if(($_GET['cof'])!='')
$q .= " and id_coffret like '%".$_GET['cof']."|%'  ";
...
?>
<tr>
	<td class="txt_title" align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>La liste des activite :</strong></td>
</tr>
<tr><td colspan="" align="center">
<input type="button" id="ulaitem0z2z3" class="button_1" onmouseover="this.style.backgroundColor='#1E90FF'" onmouseout="this.style.backgroundColor='#1874CD'"   onclick="window.location='index.php?cmd=add-activite'" value="Ajouter" />
</td>
</tr>
...
<tr><td bgcolor="#FFFFFF" colspan="4" ><img src="img/spacer.gif" width="1" height="5" alt=""></td></tr>
		...
		<tr bgcolor="#1874CD">
			...
			<td width="250" class="txt_table_head" align="center"><strong>Description</strong></td>
		...
		<tr bgcolor="#FFFFFF" onMouseOver="bgColor='#CAE5FF';" onMouseOut="bgColor='#FFFFFF';">
			<td class="txt_table" align="center"><? if ($row['image']) { ?><img width="150" height="115" src="../bimages/<?=$row['image']?>" /><? }?></td>
			<td class="txt_table" align="left"><?=substr($row['nom'],0,120)?></td>
			<td class="txt_table" align="left"><?=substr($row['description'],0,120)?></td>
			<td align="center" valign="middle">
          <? if($_GET['sous_sous_menu'] && $_GET['sous_menu'] && $i>1){?>  <a  href="javascript:void(0);"   onclick="javascript:update_list('cmd=list-activite&id=<?=$row['id']?>&sous_menu=<?=$_GET['sous_menu']?>&sous_sous_menu=<?=$_GET['sous_sous_menu']?>&pos=1&precedent=<?=$id?>&posprec=<?=$posprec?>&posact=<?=$row['pos'.$_GET['sous_menu']]?>');" title="Monter d'un niveau"><img src="img/up.png" width="16" height="16" alt="Monter d'un niveau" border="0" /></a>&nbsp;<? }?>
            <a href="index.php?cmd=add-activite&id=<?=$row['id']?>&sous_menu=<?=$_GET['sous_menu']?>&sous_sous_menu=<?=$_GET['sous_sous_menu']?>" title="Editer"><img src="img/icon-edit.gif" width="16" height="16" alt="Editer" border="0" /></a>&nbsp;
            <a href="index.php?cmd=delete-activite&id=<?=$row['id']?>" title="Effacer"><img src="img/icon-delete.gif" width="16" height="16" alt="Effacer" border="0" /></a>&nbsp;
			</td>
		</tr>
		<? 
		   $i++;
...
if ($num > $npp) {
	...
	for ($i = 1; $i <= $j; $i++) {
		...
		if ($n != $i) echo "&nbsp;<a class=link_blue  href=\"index.php?sous_sous_menu=".$_GET['sous_sous_menu']."&sous_menu=".$_GET['sous_menu']."&nom=".$_GET['nom']."&ref=".$_GET['ref']."&cmd=list-activite&n={$i}&p={$p}&o={$o}\">{$i}</a>";
	...
	if ($n != $j) echo "&nbsp;<a class=link_blue href=\"index.php?sous_sous_menu=".$_GET['sous_sous_menu']."&sous_menu=".$_GET['sous_menu']."&nom=".$_GET['nom']."&ref=".$_GET['ref']."&cmd=list-activite&n=" . ($n + 1) . "&p={$p}&o={$o}\">Suivante</a>";
